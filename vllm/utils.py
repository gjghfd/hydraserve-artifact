...
def is_pin_memory_available() -> bool:
    # TODO (chiheng): pin memory in another thread
    if int(os.getenv("FAST_COLDSTART", "1")) == 1:
        print(f"WARNING: do not pin cpu memory for faster cold start.")
        return False
...
def socket_recv(sock, blocking: bool = True) -> bytes:
    if blocking:
        length_bytes = sock.recv(8)
    else:
        try:
            length_bytes = sock.recv(8, 0x40)   # no block
        except BlockingIOError as e:
            return None
    if len(length_bytes) < 8:
        if len(length_bytes) == 0:
            raise RuntimeError("socket_recv: opponent disconnected.")
        remaining_bytes = sock.recv(8 - len(length_bytes))
        length_bytes += remaining_bytes
    length = int.from_bytes(length_bytes, byteorder='little', signed=False)
    data = bytearray(length)
    view = memoryview(data)
    cur_len = 0
    while cur_len < length:
        new_len = sock.recv_into(view[cur_len:], nbytes=length - cur_len)
        cur_len += new_len
    return data

def socket_send(sock, data: bytes):
    length = len(data)
    length_bytes = length.to_bytes(length=8, byteorder='little', signed=False)
    if length > 1000000:
        # For large data, send them subsequently to avoid byte operation
