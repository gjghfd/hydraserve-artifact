...
class Scheduler:
    ...
    def __init__(
        ...
        # Pipeline parallel batch size
        self.max_batch_size = int(os.getenv("BATCH_SIZE", "8"))
    ...
    def _schedule_running(
        ...
        while running_queue:
            ...
            if seq_group.in_process:
                # the request is in process by pipeline stages
                break
            
            if use_dest is None:
                use_dest = seq_group.use_dest
            else:
                if use_dest != seq_group.use_dest:
                    # we should group requests that all use dest model or all not use dest model together
                    break
            
            if not use_dest:
                if cur_scheduled_reqs == self.max_batch_size:
                    break
                cur_scheduled_reqs += 1
            ...
            while not self._can_append_slots(seq_group):
                ...
                victim_seq_group = None
                if running_queue and not running_queue[len(running_queue)-1].in_process:
                    victim_seq_group = running_queue.pop()
                
                print(f"Preempt happened, victim = {victim_seq_group}")
    ...
    def _schedule_prefills(
        ...
        while self._passed_delay(time.time()) and waiting_queue:
            ...
            if can_allocate == AllocStatus.LATER:
                logger.info(f"_schedule_prefills: delay a prefill request as we have not enough gpu blocks")
                break
    ...
    def _schedule_default(self) -> SchedulerOutputs:
        ...
        if not self.swapped:
            ...
            logger.error("Cannot schedule prefill due to swapped requests!")
    ...
    def _append_slots(
        ...
        for seq in seq_group.get_seqs(status=SequenceStatus.RUNNING):
            cows = self.block_manager.append_slots(seq, num_lookahead_slots, seq_group.use_dest)
