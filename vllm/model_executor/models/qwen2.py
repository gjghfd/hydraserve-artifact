...
class Qwen2Model(nn.Module):
    ...
    def __init__(
        ...
        if self.pp_rank == 0:
            if Qwen2ForCausalLM.duplicate_embed_tokens is not None:
                self.embed_tokens = Qwen2ForCausalLM.duplicate_embed_tokens
            else:
                self.embed_tokens = VocabParallelEmbedding(
                    config.vocab_size,
                    config.hidden_size,
                )
        if len(Qwen2ForCausalLM.duplicate_decoder_layer) > 0:
            layer_list = []
            for i in range(config.num_hidden_layers):
                if i in Qwen2ForCausalLM.duplicate_decoder_layer:
                    layer_list.append(Qwen2ForCausalLM.duplicate_decoder_layer[i])
                else:
                    layer_list.append(Qwen2DecoderLayer(config, i, quant_config))
        ...
        if self.pp_rank == self.pp_size - 1:
            if Qwen2ForCausalLM.duplicate_norm is not None:
                self.norm = Qwen2ForCausalLM.duplicate_norm
            else:
    ...
    def __init__(
        ...
        if self.pp_rank == self.pp_size - 1:
            if config.tie_word_embeddings:
                ...
                if Qwen2ForCausalLM.duplicate_lm_head is not None:
                    self.lm_head = Qwen2ForCausalLM.duplicate_lm_head
                else:
                ...
                for prev_name, after_name in duplicate_state_dict_json.items():
                    ...
                    if prev_layer_num == -1:
                        ...
                        if 'lm_head' in prev_name:
                            Qwen2ForCausalLM.duplicate_lm_head = prev_model.lm_head
                        elif 'embed' in prev_name:
                            Qwen2ForCausalLM.duplicate_embed_tokens = prev_model.model.embed_tokens
                        elif 'norm' in prev_name:
                            Qwen2ForCausalLM.duplicate_norm = prev_model.model.norm
                        else:
                            print(f"param name = {prev_name}")
                            raise ValueError(f"Error: unrecognized parameter.")
                    else:
                        # decoder layer
                        Qwen2ForCausalLM.duplicate_decoder_layer[dest_layer_num] = prev_model.model.layers[prev_layer_num]
