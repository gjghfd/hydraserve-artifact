...
class LlamaModel(nn.Module):
    ...
    def __init__(
        ...
        if self.pp_rank == 0:
            if LlamaForCausalLM.duplicate_embed_tokens is not None:
                self.embed_tokens = LlamaForCausalLM.duplicate_embed_tokens
            else:
        ...
        if len(LlamaForCausalLM.duplicate_decoder_layer) > 0:
            layer_list = []
            for i in range(config.num_hidden_layers):
                if i in LlamaForCausalLM.duplicate_decoder_layer:
                    layer_list.append(LlamaForCausalLM.duplicate_decoder_layer[i])
                else:
                    layer_list.append(LlamaDecoderLayer(config, quant_config))
        ...
        if self.pp_rank == self.pp_size - 1:
            if LlamaForCausalLM.duplicate_norm is not None:
                self.norm = LlamaForCausalLM.duplicate_norm
            else:
...
class LlamaForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        if self.pp_rank == self.pp_size - 1:
            if LlamaForCausalLM.duplicate_lm_head is not None:
                self.lm_head = LlamaForCausalLM.duplicate_lm_head
            else:
        ...
        for name, loaded_weight in weights:
            ...
            for (param_name, weight_name, shard_id) in stacked_params_mapping:
                ...
                if name not in params_dict:
                    print(f"WARNING: weight {name} ignored")
                    continue
                param = params_dict[name]
                ...
                for prev_name, after_name in duplicate_state_dict_json.items():
                    ...
                    if prev_layer_num == -1:
                        ...
                        if 'lm_head' in prev_name:
                            LlamaForCausalLM.duplicate_lm_head = prev_model.lm_head
                        elif 'embed' in prev_name:
                            LlamaForCausalLM.duplicate_embed_tokens = prev_model.model.embed_tokens
                        elif 'norm' in prev_name:
                            LlamaForCausalLM.duplicate_norm = prev_model.model.norm
                        else:
                            print(f"param name = {prev_name}")
                            raise ValueError(f"Error: unrecognized parameter.")
                    else:
                        # decoder layer
                        LlamaForCausalLM.duplicate_decoder_layer[dest_layer_num] = prev_model.model.layers[prev_layer_num]
