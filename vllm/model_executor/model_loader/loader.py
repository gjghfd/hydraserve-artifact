...
class DefaultModelLoader(BaseModelLoader):
    ...
    def _prepare_weights(self, model_name_or_path: str,
        ...
        if len(hf_weights_files) == 0:
            ...
            if use_local_server == 1:
                print(f"Warning: Cannot find any model weights with `{model_name_or_path}`")
                print(f"Warning: always use safetensors")
                use_safetensors = True
            else:
                raise RuntimeError(
    ...
    def load_model(self, *, model_config: ModelConfig,
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            with torch.device(torch.device("cpu")):     # From Chiheng: initialize model on CPU
            ...
            etime = time.time()
            print(f"load model: 'model.to' time cost = {etime - stime} seconds")
